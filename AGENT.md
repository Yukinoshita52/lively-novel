...
- 以阶段性纵向切片推进；允许围绕同一目标大阔步合并多个相关能力，但每个阶段合入后都应端到端可演示。
...
- 允许每次大阔步推进一个阶段性能力包，但提交和 PR 必须有清晰边界、可验证结果和可回滚范围。
- PR 默认目标分支是 `master`。
...
- 一个 PR 只做一个明确阶段目标；允许包含多个强相关能力，但不能混入无关改动。
- 一个 PR 内可拆多个 commit，但每个 commit 只承担单一职责
- 默认按“配置 / 文档 / 测试 / 功能接口”或“阶段目标下的能力模块”拆分；若依赖关系导致必须调整，先说明原因。
...
- 后续 Agent 化改造基于 `master` 分支推进；每个阶段性切片都应保持稳定主链路可运行，允许大步推进但不降低验证门槛。
