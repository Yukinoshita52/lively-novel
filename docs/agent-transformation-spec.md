...
实施节奏调整为“大切片 + 阶段门”：允许一次 PR 或一个开发周期覆盖多个强相关 Agent 能力，但必须服务于同一阶段目标，并满足以下阶段门：

- 主链路不退化：导入、转换、预览、打磨、导出仍可运行。
- 边界可审查：PR 描述必须说明本次阶段目标、包含能力和明确非目标。
- 状态可验证：新增 trace、工具、planner、guardrail 或前端能力必须有对应自动化测试或可观察手动验证点。
- 风险可回滚：不把无关重构、样式调整、实验性 MCP/Skill 与当前阶段目标混在同一 PR。
...
下表不再要求严格一项一 PR，可按阶段目标合并相邻强相关项。例如 `PR-1` 到 `PR-4` 已可合并为一个 Phase 1 基础骨架 PR；后续也可把 `AgentStep + Guardrail + 细粒度工具拆分` 作为一个更大的阶段性 PR。合并前提是实现边界清晰、测试覆盖足够、主链路不退化。
